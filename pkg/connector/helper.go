...
// disappears from a later sync and gets treated as fully deleted. Five of them
// tolerate this error on the first page of List() (see call sites), which is what
// makes unconditional registration safe for those five: the sync skips that one
// resource type gracefully instead of failing outright. clm_workflow_queue is the
// deliberate exception — it's registered unconditionally the same way, but does not
// tolerate this error; see clm_workflow_queues.go's List() doc for why it fails loud
// instead.
//
