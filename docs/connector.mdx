...
*Signing groups sync automatically whenever the account has the signing groups feature enabled — no configuration needed. The **Include Signing Groups** setting is deprecated and kept only for backward compatibility with existing configurations; it has no effect.

**DocuSign CLM (Contract Lifecycle Management) is a separate, separately-licensed DocuSign product. CLM resources sync automatically once the **Include CLM** setting is enabled (it requests the additional OAuth scopes CLM needs) and the account has a DocuSign CLM production subscription. CLM permission sets sync for visibility only; DocuSign's CLM API has no endpoint to assign or unassign one.
...
- If enabling **Include CLM**: your DocuSign account needs a CLM production subscription, and the credential used must have read (and, for folder security and CLM group provisioning, write) access to CLM folders, groups, and members.
</Warning>
...
<Steps>
  ...
  **If you selected Custom App (Demo Environment):** Enter the **Client ID** and **Client Secret** from your DocuSign developer app.
  ...
  <Step>
  Signing groups sync automatically whenever your account has the feature enabled — no setup needed here. (The **Sync signing groups** toggle shown in older setups is deprecated and has no effect.)
  </Step>
  <Step>
  **Optional.** If your DocuSign account has a CLM (Contract Lifecycle Management) production subscription and you want to review CLM folders, folder security, groups, and permission sets alongside eSignature, click to enable **Include CLM**.

  <Warning>
  If you selected **OAuth Authentication** (the default, managed method) above, enabling **Include CLM** also requires ConductorOne's managed OAuth app to be granted the CLM API scope on the platform side. If CLM data doesn't appear after completing setup, contact ConductorOne. This limitation doesn't apply to **Custom App (Demo Environment)**, where the connector requests the CLM scope directly using your own DocuSign app credentials.
  </Warning>
  </Step>
  <Step>
...
stringData:
  ...
  # Signing groups sync automatically whenever the account has the feature enabled —
  # no configuration needed. BATON_INCLUDE_SIGNING_GROUPS is deprecated and has no effect.

  # Optional: include if you want C1 to sync DocuSign CLM (Contract Lifecycle
  # Management) folders, folder security, groups, and permission sets. Requests the
  # additional OAuth scopes CLM needs; also requires a DocuSign CLM production
  # subscription on the account.
  BATON_INCLUDE_CLM: true
