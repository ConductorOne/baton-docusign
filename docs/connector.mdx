...
**DocuSign CLM (Contract Lifecycle Management) is a separate, separately-licensed DocuSign product. CLM resources are not synced by default — enable them with the **Include CLM** setting, which requires a DocuSign CLM production subscription. CLM permission sets sync for visibility only; DocuSign's CLM API has no endpoint to assign or unassign one.
...
stringData:
  ...
  # Management) folders, folder security, groups, and permission sets.
