...
<Steps>
  ...
  In the **Redirect URI** field:
    - If you're configuring a **self-hosted** connector, enter any URI of your choice (such as http://example.com/callback) and click **Add**. The `--configure` flow prints an authorization URL and reads the resulting code from the terminal — it never receives a redirect, so this value is never actually used.
